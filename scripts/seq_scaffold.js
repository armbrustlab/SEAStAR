...
(function() {
  ...
  */
  ...
  process_scaffs = function(fasta) {
    var c, cf, cfmax, contig, cr, crmax, frame, i, j, l, m, match, max, n, name, ns, num_contigs, overlap, pad, pf, pfmax, pr, prev_contig, prmax, r, rc, rev_contig, rev_prev_contig, rp, s, scaf, scaf_ind, scaff_cnt, scaff_names, scaff_num, scaff_nums, scafout, seqs, trim_match, x, y, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _m, _n, _o, _p, _q, _r, _ref, _ref1, _ref10, _ref11, _ref12, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9, _results, _s, _t, _u, _v, _w;
    num_contigs = [];
    scaff_cnt = 0;
    scaff_nums = {};
    scaff_names = [];
    ...
    m = overlap_m;
    max = overlap_max;
    for (x = _j = 0, _len1 = scaffs.length; _j < _len1; x = ++_j) {
      scaf = scaffs[x];
      if (scaf) {
        for (y = _k = 0, _len2 = scaf.length; _k < _len2; y = ++_k) {
          ...
          for (i = _l = max; max <= n ? _l <= n : _l >= n; i = max <= n ? ++_l : --_l) {
            c = 0;
            ...
            if ((c >= n) && (c >= i - (2 * m))) {
              ...
              if (verbose) {
                ...
                pad = '';
                for (x = _n = 1, _ref2 = max - i + (j - c); 1 <= _ref2 ? _n < _ref2 : _n > _ref2; x = 1 <= _ref2 ? ++_n : --_n) {
                  pad = pad + " ";
                }
                console.warn("%s%s", pad, overlap);
                pad = '';
                for (x = _o = 1, _ref3 = max - i; 1 <= _ref3 ? _o < _ref3 : _o > _ref3; x = 1 <= _ref3 ? ++_o : --_o) {
                  pad = pad + " ";
                }
    ...
    for (x = _p = 0, _len3 = scaffs.length; _p < _len3; x = ++_p) {
      scaf = scaffs[x];
      if (scaf) {
        scaffs[x] = (function() {
          var _len4, _q, _results;
          _results = [];
          for (_q = 0, _len4 = scaf.length; _q < _len4; _q++) {
            contig = scaf[_q];
            if (contig != null) {
    ...
    if (heal_seq) {
      ...
      m = heal_m;
      max = heal_max;
      for (scaf_ind = _q = 0, _len4 = scaffs.length; _q < _len4; scaf_ind = ++_q) {
        scaf = scaffs[scaf_ind];
        if (scaf) {
          for (y = _r = 0, _len5 = scaf.length; _r < _len5; y = ++_r) {
            contig = scaf[y];
            if (!(y)) {
              continue;
            }
            ...
            if (match = heal_seq.match(r)) {
              ...
              if (verbose) {
                ...
                for (x = _s = 1, _ref4 = n + overlap.length - m; 1 <= _ref4 ? _s <= _ref4 : _s >= _ref4; x = 1 <= _ref4 ? ++_s : --_s) {
                  pad = pad + " ";
                }
    ...
    for (x = _t = 0, _len6 = scaffs.length; _t < _len6; x = ++_t) {
      scaf = scaffs[x];
      if (scaf) {
        scaffs[x] = (function() {
          var _len7, _results, _u;
          _results = [];
          for (_u = 0, _len7 = scaf.length; _u < _len7; _u++) {
            contig = scaf[_u];
            if (contig != null) {
              _results.push(contig);
            }
    ...
    ns = ['', 'N', 'NN'];
    _results = [];
    for (x = _u = 0, _len7 = scaffs.length; _u < _len7; x = ++_u) {
      scaf = scaffs[x];
      ...
      for (y = _v = 0, _len8 = scaf.length; _v < _len8; y = ++_v) {
        contig = scaf[y];
        if (!(y)) {
          continue;
        }
        ...
        prmax = [-1, -1];
        crmax = [-1, -1];
        for (n = _w = 0; _w <= 2; n = ++_w) {
          pf = (_ref5 = (_ref6 = (prev_contig + ns[n]).match(rp)) != null ? _ref6[0].length : void 0) != null ? _ref5 : 0;
          if (pf > pfmax[1]) {
            pfmax = [n, pf];
          }
          cf = (_ref7 = (_ref8 = (ns[n] + contig).match(rc)) != null ? _ref8[0].length : void 0) != null ? _ref7 : 0;
          if (cf > cfmax[1]) {
            cfmax = [n, cf];
          }
          pr = (_ref9 = (_ref10 = (ns[n] + rev_prev_contig).match(rc)) != null ? _ref10[0].length : void 0) != null ? _ref9 : 0;
          if (pr > prmax[1]) {
            prmax = [n, pr];
          }
          cr = (_ref11 = (_ref12 = (rev_contig + ns[n]).match(rp)) != null ? _ref12[0].length : void 0) != null ? _ref11 : 0;
          if (cr > crmax[1]) {
            crmax = [n, cr];
          }
        }
