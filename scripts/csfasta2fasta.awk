...
# Input:   One or two CSFASTA files (if two, they must contain read2 and read1) 
# Output:  stdout

# Variables R1 and R2 control whether read1 and read2 are reversed, respectively.
# By default, read1 is reversed (R1 = 1) and read2 is not (R2 = 0) 
# These may be overridden on the cmd line e.g. -v R1=0
...
BEGIN { 
	...
	a["."] = "N"; 
        ...
        if ((ARGV[2]) && ((ARGV[1] !~ /read2/) || (ARGV[2] !~ /read1/))) {
           print "Invalid paired input files. Must be <read2> <read1>" > "/dev/stderr"
           exit(1)
        }
